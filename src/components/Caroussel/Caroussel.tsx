...
const Carousel: React.FC<CarouselProps> = ({ comics }) => {
    ...
    const handleCarouselClick = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        const clickedIndex: number = Math.floor((e.clientX - carouselRef.current!.getBoundingClientRect().left) / cardWidth);
    ...
    return (
        <div className="carousel-container" ref={carouselRef} onClick={handleCarouselClick} onScroll={handleScroll}>
            <div
                className="carousel"

            >
                {comics && comics.map((comic: Comic, index: number) => (
                    <ComicCard cardRef={cardRef} key={index} imageUrl={createMarvelImg({ path: comic.thumbnail.path, extension: comic.thumbnail.extension })} name={comic.title} handleClick={() => handleCardClick(index)} />
                ))}
            </div>
